...
### metadata
metadata_filename = 'General\\dimer_metadata_file.xlsx'
...
plot_VVAcal = False
data_file = os.path.join(root + '\\General\\VVAtoVpp_square_43p2MHz.txt')
...
# 	Ebfix = x[np.argmax(y)] 
	...
	if not load_lineshape:
		...
		for xconv in xxC:
			...
			yyconv.append(a)
			e_yyconv.append(b)
		convnorm = np.trapezoid(yyconv,xxC)
...
# 		ax_ls.errorbar(xnfilt, ynfilt, yerrnfilt, marker='o', ls='', markersize = 12, capsize=3, mew=3, mfc='none', color='tab:gray', elinewidth=3)
	...
	if fitWithOffset:
		SR_convls = np.trapezoid(yyconvls - popt[-1], xx)
		FM_convls = np.trapezoid((yyconvls - popt[-1])* xx, xx)
		fitdof = 3
	else:
		SR_convls = np.trapezoid(yyconvls, xx)
		FM_convls = np.trapezoid(yyconvls*xx, xx)
